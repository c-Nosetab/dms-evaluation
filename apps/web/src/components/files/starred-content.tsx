...
export function StarredContent({ userName }: StarredContentProps) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [filesResponse, foldersResponse] = await Promise.all([
        fetch(`${apiUrl}/files/starred`, { credentials: 'include' }),
        fetch(`${apiUrl}/folders/starred`, { credentials: 'include' }),
      ]);
  ...
  const handleToggleFileStar = useCallback(async (fileId: string) => {
    try {
  ...
  const handleToggleFolderStar = useCallback(async (folderId: string) => {
    try {
      ...
      if (!data.isStarred) {
        ...
        setFolders((prev) =>
          prev.map((f) => (f.id === folderId ? { ...f, isStarred: data.isStarred } : f))
        );
        setInfoFolder((prev) =>
    ...
    <div className="space-y-8">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-(--muted-foreground) max-w-md mx-auto leading-relaxed">
                  Star your favorite files and folders to access them quickly from here.
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <RenameDialog
        ...
        onRename={async (newName) => {
          if (renameFile) {
            await handleRenameFile(renameFile.id, newName);
          }
        }}
      />
      ...
      <RenameDialog
        ...
        onRename={async (newName) => {
          if (renameFolder) {
            await handleRenameFolder(renameFolder.id, newName);
          }
        }}
      />
    </div>
