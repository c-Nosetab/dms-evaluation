...
export function DashboardContent({ userName }: DashboardContentProps) {
  ...
  const loadData = useCallback(async (folderId: string | null) => {
    try {
      ...
      const newFiles = filesResponse.ok ? (await filesResponse.json()).files || [] : [];
      const newFolders = foldersResponse.ok ? (await foldersResponse.json()).folders || [] : [];
  ...
  const loadBreadcrumb = useCallback(async (folderId: string) => {
    try {
      const response = await fetch(`${apiUrl}/folders/${folderId}/breadcrumb`, {
        credentials: 'include',
      });
      if (response.ok) {
  ...
  const handleFolderItemClick = useCallback((folder: FolderItem) => {
  ...
  const handleFolderRename = useCallback(async (folderId: string, newName: string) => {
    ...
    setFolders((prev) =>
      prev.map((f) => (f.id === folderId ? { ...f, name: updatedFolder.name, updatedAt: updatedFolder.updatedAt } : f))
    );
    setInfoFolder((prev) =>
      prev?.id === folderId ? { ...prev, name: updatedFolder.name, updatedAt: updatedFolder.updatedAt } : prev
    );
  ...
  const handleFolderRemove = useCallback(async (folderId: string) => {
    ...
    try {
      ...
      if (currentFolderId === folderId) {
        const parent = breadcrumb[breadcrumb.length - 2] || { id: null, name: 'My Files' };
  ...
  const renderFolderMenuItems = (folder: FolderItem, MenuItemComponent: typeof DropdownMenuItem | typeof ContextMenuItem, SeparatorComponent: typeof DropdownMenuSeparator | typeof ContextMenuSeparator) => (
    <>
  ...
  const renderFileMenuItems = (file: FileItem, MenuItemComponent: typeof DropdownMenuItem | typeof ContextMenuItem, SeparatorComponent: typeof DropdownMenuSeparator | typeof ContextMenuSeparator) => (
    <>
  ...
  return (
    <UploadZone onFilesSelected={handleFilesSelected} className="min-h-full">
      <div className="space-y-8">
            ...
            <div className="md:hidden flex items-center gap-1">
              <DropdownMenu>
                <DropdownMenuTrigger className="flex items-center gap-1 px-2 py-1 rounded-md hover:bg-(--muted) text-(--muted-foreground) hover:text-(--foreground) transition-colors cursor-pointer">
                  ...
                  <span className="max-w-[120px] truncate">{breadcrumb[breadcrumb.length - 1]?.name}</span>
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="hidden md:flex items-center gap-1">
              {breadcrumb.length <= 3 ? (
                ...
                <>
                  {/* First item (My Files) */}
                  <button
                    onClick={() => handleBreadcrumbClick(breadcrumb[0]?.id ?? null)}
                  ...
                  <button
                    onClick={() => handleBreadcrumbClick(breadcrumb[breadcrumb.length - 1]?.id ?? null)}
        ...
        {/* Quick actions */}
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 ${hasAnimated ? '' : 'animate-fade-up delay-100'}`}>
          <Card
        ...
        {files.length > 0 || folders.length > 0 ? (
          ...
          <Card className={`border-2 border-dashed border-(--primary)/20 bg-gradient-cream ${hasAnimated ? '' : 'animate-fade-up delay-200'}`}>
